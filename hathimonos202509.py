# -*- coding: utf-8 -*-
"""HathiMonos202509.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1l-rgPjuFcJNoOMVD8PFqpXqTSQy36vSC
"""
...
conn = sqlite3.connect("HathiMonos2025.db")

cursor = conn.cursor()

cursor.execute("DROP TABLE IF EXISTS HathiMonos2025")

pip install pymarc

"""The following has cleaned 533's, removing extra iterations for non-a subfields.  Close enough length, confirmed that multiple 533's are recorded."""

import csv

csv_out = csv.writer(open('/content/drive/My Drive/Data Services/HathiTrustHoldingsLoad/2025/SeptVersion/MonoProtoBibs.csv', 'w'), delimiter = ',', quotechar = '"', quoting = csv.QUOTE_MINIMAL)
...
Bibs = pd.read_csv('/content/drive/My Drive/Data Services/HathiTrustHoldingsLoad/2025/SeptVersion/MonoProtoBibs.csv', names=colnames, low_memory=False)
...
csv_out = csv.writer(open('/content/drive/My Drive/Data Services/HathiTrustHoldingsLoad/2025/SeptVersion/MonoProtoLHRsHurry.csv', 'w'), delimiter = ',', quotechar = '"', quoting = csv.QUOTE_MINIMAL)
...
LHRs = pd.read_csv('/content/drive/My Drive/Data Services/HathiTrustHoldingsLoad/2025/SeptVersion/MonoProtoLHRs.csv', names=colnamesc, low_memory=False)
...
LHRs = pd.read_sql("""SELECT [LOCN], [LCN] FROM LHRs WHERE [LHRLDR06] Like 'x' AND [Location] Not Like '%Micro%'""", conn)

LHRs.to_sql('LHRs', conn, if_exists='replace', index = False)

spm = pd.read_sql("""SELECT DISTINCT(Bibs.[OCN]) AS oclc, LHRs.[LCN] AS local_id, [field00828] AS govdoc
...
spm['govdoc'] = spm['govdoc'].replace(['f'], '1')

print(spm['govdoc'].unique())

spm['govdoc'] = spm['govdoc'].replace([' ', 's', 'i', 'l', 'o', 'm', 'c', 'z', 'u', '-', 'a', 'r'], '0')

print(spm['govdoc'].unique())

spm['oclc'] = spm['oclc'].str.replace('\\','')

print(spm)

spm.to_csv('/content/drive/My Drive/Data Services/HathiTrustHoldingsLoad/2025/SeptVersion/wvu_spm_full_202509.tsv', sep = '\t', index=False)